...
const perspectivePrompt = ChatPromptTemplate.fromTemplate(
    `당신은 AI언어모델 어시스턴트입니다. 주어진 사용자 다섯 가지 버전을 생성하여 벡터데이터베이스에서 관련문서를 검색하세요. 사용자 질문에 대한 다양한 관점을 생성함으로써 ㅅ ㅏ용자가 거리기반 유사도 검색의 한계를 극복할수있도록 돕는것이 목표입니다. 이러한 대체 질문을 개행으로 구분하여 제공하세요. 원래질문 : {question}`
)
...
const result = await multiQueryQa.invoke('현존하는 LLM중 어떤모델이 코딩,창작에 제일 높은 성능을 보이나요?')
